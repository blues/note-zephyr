...
To simplify the process of developing Zephyr applications, we support two workflows, `native` and `devcontainer`. These are both designed to be used within VS Code.

## Native

The `native` workflow allows you to develop Zephyr applications on your host machine, managed by VS Code.
...
The `devcontainer` workflow allows you to develop Zephyr applications within a containerized environment, managed by VS Code. To read more about the devcontainer, please see the [Devcontainer](./devcontainer.md) section.
